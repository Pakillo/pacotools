...
#' @export
#' @return A plot
#' @param predictor The predictor. A numeric vector.
#' @param response The response variable. A numeric vector (can be a binary variable).
...
#' @param BETA A numeric matrix of nsims x nparams dimensions with the posterior values of the model parameters, e.g.
#' as retrieved from model$BUGSoutput$sims.matrix[,c("intercept", "slope")]
#' @param XXMATRIX A numeric matrix with ncol=nparams, containing the values of the covariates that should be used for the plot,
#' including the intercept. The order of the covariate columns should be the same as the parameter columns in BETA.
...
#' @author Paco, based on code by Lucas Leeman (see http://andrewgelman.com/2012/08/graphs-showing-regression-uncertainty-the-code/)
#' 
#'
#' 
#' @examples
...
#' BETA <- cbind(interc.post, slope.post)
#' XXMATRIX <- cbind(rep(1,length(predictor)), predictor)


plotReg <- function(predictor, response, BETA, XXMATRIX, plot.unc=T, spag=T, colour, alpha.spag=1, lwd.spag=0.1, 
  ...
  if (add==F){
    ...
    plot(predictor,
         response,
         las=1, lwd=2, pch=20, cex=0.5, cex.axis=1.2, cex.lab=1.2, ...)
  }

  predvals <- seq(min(predictor), max(predictor), length.out=100)
  ...
  if (plot.unc==T){
    ...
    fittedy <- BETA %*% t(XXMATRIX)   # Modelled response values
    if (length(unique(response))==2){  # if the response is a binary variable (ie. logistic regression)
      fittedy <- pnorm(fittedy)
